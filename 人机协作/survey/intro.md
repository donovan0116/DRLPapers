1. 自动化所，解决大规模baseLLM的agent的决策制定，其中agent的数目不固定。为了解决合作问题。使用AC框架，中心化critic，分布式actor. Critic做了创新，原来的一个Critic变成了一个Critic组合，其中的每个小Critic负责reward的一部分，汇总让总Critic向每个Actor发送建议。Actor可以选择接受建议或不接受。对通信的创新体现在：首先直接给Actor发送策略变成了发送建议，由Actor来判断，体现了对RL算法improve的提升；其次，对Critic方法的创新（与通信无关）。这体现了通信对MARL系统的RL的算法评估的提升。

2. natural language-based reinforcement learning，引入task language概念，将nl翻译为agent能够理解的tl，然后执行，并且具备tl生成的模块。设计AI与人类的通信管道TL。属于通信辅助RL动作选择。侧重于动作选择中的improve

3. 腾讯AIlab，训练新的联盟学习玩星际，使得智能体有目的性的挖掘自身联盟的弱点（在星际中，不完全或不完美信息可以被侦察出来，与麻将不同），并激励智能体更倾向于侦察，通过更少的资源训练更好的AI。（跟通信没有任何关系）
...
36. ICML 2020 Facebook AI。算法名称为Other-play，利用马尔科夫博弈中的已知对称性，寻找更强大的策略来增强self-play。具体的思路：原本的Self-Play是和自己玩游戏，这样学到的策略都是自己的策略。OP的思想是，在原有self-play的基础上，加入一定的对称性映射，具体方式是构建一个对称性映射的函数族，这种函数族形成的映射和原有的SAR对是同构的，因此也能训出来鲁棒的策略。对称性映射有点类似于SL中扩展数据集的方法，把数据集图片左右翻转，对称性映射将观察、状态、动作和汇报都用同样的对称性映射了（如本来往左走，镜像对称成往右走）。这样的训练增强了zero-shot 的能力。（看到第44进行补充：在纯SP中，智能体之间会对环境构成一定的协定。通过对称性变换环境，可以打破这种协定。）
...
44. 麦吉尔大学 ICML 2021，zero-shot 合作的轨迹多样性。通过SP训练的智能体在遇到没见过的智能体时非常疑惑。本文提出一种可微的目标函数(TrajeDi)用来生成多样性的策略。整体来讲可以为整个agent的种群构建BR。（一种评价ZSC的指标：cross-play，具体来讲就是构建两个独立训练的种群，分别采样agent）之前的工作中，ZSC一般是对动作函数的随机化，状态分布的随机和状态-动作分布的随机。本文首次使用轨迹随机化的研究来研究随机化。

    ![image-20240614102402098](./intro.assets/image-20240614102402098.png)

    关于common BRs和trajectory diversity的解释：https://chatgpt.com/share/5793745a-9951-43e5-9b89-5fb494d2774d

    common BR有好处，首先，每个智能体都有自己的策略，为了进行cross-play，我们训练出来一个common BR，这个common BR与每个策略共同训练而成，能够和所有策略都协调的很好。最后进行cross-play的时候，每个智能体只需要从common BR中得到策略就好了。优点就是：这样和直接协调相比，每个智能体不需要看到所有其他的策略，只需要知道怎么应对就好了。

45. AAMAS 2022，deepmind，研究感情指标（温情和可信度）对于人机协作中人对智能体的选择偏好的影响。实验表明，感情影响比客观指标来得更准。

46. ICCV 2017 佐治亚理工，开发了一个端到端的视觉问答机器人训练框架。两个agent分别负责问和答，answer方有一个图片，question有一个图片库，猜这个图片是什么。其中question机器人可以提出问题，answer来回答。双方之间交流使用的是自然语言的离散化符号组成的通信协议。